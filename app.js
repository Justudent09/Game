...
tg.SettingsButton.onClick(() => {
    tg.showConfirm("Вы уверены, что хотите полностью сбросить прогресс турнира?", (isConfirmed) => {
        if (isConfirmed) {
            ...
            tg.showAlert("Данные удалены.", () => location.reload());
        }
    });
});
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function saveState() {
        ...
        if (tg.CloudStorage) tg.CloudStorage.setItem(STORAGE_KEY, dataToSave);
    }
    ...
    if (savedData) {
        ...
        players = [...TOURNAMENT_PLAYERS].sort(() => Math.random() - 0.5);
    }
    ...
    function createMatch(x, y, aName, bName, isChampion=false, matchId) {
        ...
        if (isChampion) {
            el.innerHTML = `<div class="row" style="font-size:10px; opacity:0.5;">ЧЕМПИОН</div><div class="row ${classA}" id="${matchId}-0" ${styleA}>${contentA}</div>`;
        } else {
            el.innerHTML = `<div class="row" id="${matchId}-0" ${styleA}>${contentA}</div><div class="row" id="${matchId}-1" ${styleB}>${bName || ""}</div>`;
        }

        if (matchId !== "CHAMP") el.onclick = () => openTelegramPopup(matchId);
    ...
    function openTelegramPopup(id) {
        ...
        tg.showPopup({ title: 'Результат матча', message: 'Кто проходит дальше?', buttons }, (btnId) => {
    ...
    function setWinner(matchId, name) {
        ...
        if (m.nextMatchId) {
            ...
            if (nextMatchEl && nextMatchEl.querySelectorAll('.row').length === 1 && m.nextMatchId !== "CHAMP") setWinner(m.nextMatchId, name);
        }
        saveState();
    }
    ...
    function run(list) {
        ...
        for (let i = 0; i < list.length; i += 2) {
            const a = list[i], b = list[i+1] || null, mid = `r${round}m${i}`;
            const el = createMatch(0, i * stepY, a, b, false, mid);
            matchData[mid] = { el, nextMatchId: null, nextSlot: 0 };
            current.push({ el, x: 0, mid });
        ...
        while (current.length > 1) {
            ...
            for (let i = 0; i < current.length; i += 2) {
                ...
                const el = createMatch(round * stepX, 0, "None", B ? "None" : null, false, mid);
                matchData[mid] = { el, nextMatchId: null, nextSlot: 0 };
                const cA = getCenterY(A.el), cB = B ? getCenterY(B.el) : cA, center = (cA + cB) / 2;
                el.style.top = (center - el.offsetHeight / 2) + "px";
                const sX = A.x + cardW, mX = sX + 30, eX = round * stepX;
                drawH(sX, cA, 30);
                ...
                next.push({ el, x: round * stepX, mid });
            }
            current = next; round++;
        }
        ...
        if (current.length === 1) {
            ...
            const el = createMatch(round * stepX, 0, "None", null, true, mid);
    ...
    function updateScaling() {
        ...
        const safeTop = parseFloat(getComputedStyle(wrapper).marginTop) || 0;
        
        const availW = vw - (padding * 2);
        const availH = vh - safeTop - (padding * 2);

        const scale = Math.min(availW / contentW, availH / contentH);
        wrapper.style.transform = `scale(${scale})`;
        
        document.querySelector('.scroll-area').style.paddingBottom = (padding * 2) + 'px';
    }
